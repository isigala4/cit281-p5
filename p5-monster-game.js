...
module.exports = class MonsterGame {
  ...
  // List monsters
  listMonsters(){
  this.monsters.forEach((monster) => console.log(`monsterName: ${monster.monsterName} `+ `minimumLife: ${monster.minimumLife} `+ 
  `currentLife: ${monster.currentLife} ` +` Status: ${monster.isAlive ? 'Alive' : 'Dead'}`));
  };
...
listPowerDrain(){
    this.monsters.forEach((monster) => {
       const powerdrain =monster.randomLifeDrain(this.minimumLifeDrain, this.maximumLifeDrain);
       console.log(`${monster.monsterName} random power drain of ${powerdrain}`);
       //cant figure out how to call the same value that is generated from the original randomLifeDrain,
       //and so the console outputs dont match 
});
}
